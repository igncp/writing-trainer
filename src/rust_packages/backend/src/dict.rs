...
use crate::env::ENV_DICT2_FILE_PATH;
...
fn parse_dict_1() -> Result<ParsedDict, Box<dyn Error>> {
...
fn parse_dict_2() -> Result<ParsedDict, Box<dyn Error>> {
    let file_path = env::var(ENV_DICT2_FILE_PATH)?;
    let file_content = std::fs::read_to_string(file_path)?;
...
pub async fn use_dict(text: &str, language: &str) -> Result<DictMatches, Box<dyn Error>> {
    ...
    let dict = parse_dict_1()?;
    let dict2 = parse_dict_2()?;
    ...
    for start in 0..text.len() {
        'loop2: for end in (start + 1)..(start + MAX_CHARS_IN_WORD) {
            ...
            if dict2.contains_key(word) {
                let has_it_already = words.iter().any(|w| w.word == word);

                if !has_it_already {
                    let extra_meaning = dict2.get(word).unwrap().to_string();
